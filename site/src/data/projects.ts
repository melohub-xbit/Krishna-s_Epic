...
const RAW: Project[] = [
  ...
  },
  {
    ...
    mark: "book",
    figure: "O(1)",
    caption: "cancellation, through a hash map written from scratch. The old design searched a tree.",
    problem:
      "The first version stored one order per price node in an AVL tree, so a second resting order at the same price was silently dropped — a book needs a FIFO queue at each price, not a single slot. Cancellation was an O(N) search, and prices were doubles.",
    approach:
      "Rewritten from scratch as a v2. A price-level map with a FIFO queue at every level, fixed-point int64 ticks instead of floating point, a from-scratch Robin Hood hash map from order id to location for O(1) cancels, a freelist pool allocator, and a lock-free SPSC ring buffer across a three-stage pipeline. Benchmarked against both a corrected port of the old AVL design and a naive map-based book on identical synthetic flow.",
    details: [
      { label: "Book", value: "Price levels, FIFO each" },
      { label: "Cancel", value: "O(1) average" },
      { label: "Prices", value: "Fixed-point int64 ticks" },
      { label: "Allocation", value: "Freelist pool" },
      { label: "Concurrency", value: "Lock-free SPSC, 3 stages" },
      { label: "Order flow", value: "Cont-Stoikov-Talreja" },
      { label: "Harness", value: "GoogleTest · Google Benchmark" },
    ],
